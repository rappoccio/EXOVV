...
jackknife_mc = ROOT.TFile('jackknife.root')
supplementaryMC = ROOT.TFile('qcdmc_stitched_qcdmc.root')
datafile = ROOT.TFile('jetht_40pbinv_weighted_dataplots.root')
responses = []
unfolded = []
...
def unfold_jackknife( unfoldedlist, pickleoutputname):
    ...
    for i, hist in enumerate(unfoldedlist):
        for j in range(0, 19):
            pt_projections[i].append(hist.ProjectionY('jackknifesample'+str(i)+str(j), j+1, j+1))
    del unfoldedlist[:]
