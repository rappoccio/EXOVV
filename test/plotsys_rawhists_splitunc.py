...
for iptbin in xrange( len(dataprojs) ) :
    ...
    for imbin in xrange( minmassbin,dataprojs[iptbin].GetNbinsX()+1 ) :
        ...
        valdata = dataprojs[iptbin].GetBinContent( imbin )
        ...
        if abs(val) > 1e-20 :
            ...
            if abs(valdata) > 1e-20 : 
                errstatdata = dataprojs[iptbin].GetBinError ( imbin ) / valdata
            else :
                errstatdata = 0.0
            errmc = errstat**2
            errdata = errstatdata**2
            ...
            errmc += pdf**2 + psunc**2
            errdata += jec**2 + jer**2 + jmr**2 + pu**2
            errmc = sqrt(errmc) * val
            errdata = sqrt(errdata) * valdata
            #print ' %6.2e +- %6.2e' % ( valdata, errdata )


        pyprojs[iptbin].SetBinError( imbin, errmc )
        hwprojs[iptbin].SetBinError( imbin, 1e-12 )
        dataprojs[iptbin].SetBinError( imbin, errdata )
